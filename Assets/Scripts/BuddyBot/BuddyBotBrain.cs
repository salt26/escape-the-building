...
public class BuddyBotBrain : MonoBehaviour
{
    ...
    public void GoTo(Vector3 position, bool useTargetHeight)
    {
        ...
        if (!NavMesh.CalculatePath(myGroundInfo.point, targetGroundInfo.point, NavMeshLayerMask, path))
        {
    ...
    private bool FindRandomPositionNearFriend(out Vector3 result)
    {
        ...
        NavMeshHit friendNavMeshInfo;
        NavMesh.SamplePosition(Friend.position, out friendNavMeshInfo, 5f, -1);
        ...
        for (int i = 0; i < 10; ++i)
        {
            ...
            NavMeshHit candidatePointInfo;
            if (NavMesh.Raycast(friendNavMeshInfo.position, result, out candidatePointInfo, -1))
